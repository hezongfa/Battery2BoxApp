...
//    }
    ...
    public static String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(Glide.getPhotoCacheDir(AppContextBase.getInstance())));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public static boolean cleanCatchDisk() {
        return deleteFolderFile(Glide.getPhotoCacheDir(AppContextBase.getInstance()).getAbsolutePath(), true);
    }
    ...
    public static boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(AppContextBase.getInstance()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
